-- Hardening RLS policies and revoking EXECUTE permissions on SECURITY DEFINER functions

-- 1. Tighten profiles SELECT: users see own profile; admins/managers see all
...
-- 2. Tighten user_roles SELECT: users see own roles; admins see all
...
-- 3. Lock audit_logs INSERT to actor = auth.uid()
...
-- 4. Restrict audit_logs SELECT to admins
...
-- 5. Revoke EXECUTE on SECURITY DEFINER helpers from authenticated; keep server-side use
REVOKE EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.is_writer(uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.handle_new_user() FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.set_updated_at() FROM PUBLIC, anon, authenticated;

-- 6. Storage RLS for private 'dani' bucket: only owner can access their files
...
USING (bucket_id = 'dani' AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "dani_insert_own" ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'dani' AND (storage.foldername(name))[1] = auth.uid()::text);
