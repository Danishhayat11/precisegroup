...
export default function BookingDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { booking: b.data, ledger: ledger.data ?? [], payments: pays.data ?? [], adjustments: adj.data ?? [] };
    },
  });
  ...
  if (!data.booking) return (
    <div className="card-elevated p-10 text-center">
      ...
      <Link to="/bookings" className="text-primary text-sm hover:underline mt-2 inline-block">← Back to bookings</Link>
    </div>
  );
  const b = data.booking;

  const today = new Date().toISOString().slice(0, 10);
  const ledgerWithStatus = data.ledger.map((l: any) => {
  ...
  return (
    <div>
      <Link to="/bookings" className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center gap-1 mb-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={k.l} className="card-elevated p-4">
            ...
            <div className={`text-lg font-semibold mt-1 tabular-nums ${k.tone === "danger" ? "text-destructive" : ""}`}>{k.v}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card-elevated overflow-hidden mb-6">
        <div className="p-4 border-b text-sm font-semibold">Installment Ledger ({ledgerWithStatus.length} terms)</div>
          ...
          <table className="w-full text-sm table-sticky">
            ...
            <tbody>
              {ledgerWithStatus.map((l: any) => (
                <tr key={l.ledger_id} className={`border-t ${l._status === "Overdue" ? "bg-destructive/5" : ""}`}>
                  ...
                  <td className="px-4 py-2 text-right tabular-nums font-medium">{fmtPKR(l._remaining)}</td>
                  <td className="px-4 py-2">{fmtDate(l.paid_date)}</td>
                  <td className="px-4 py-2 text-right tabular-nums">{l._days || "—"}</td>
                  <td className="px-4 py-2"><StatusBadge label={l._status} tone={statusTone(l._status)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {data.payments.map((p: any) => (
                <tr key={p.receipt_no} className="border-t">
                  ...
                  <td className="px-4 py-2"><StatusBadge label={p.payment_mode} tone={p.payment_mode === "Adjustment" ? "adjustment" : "info"} /></td>
                  ...
                  <td className="px-4 py-2 text-right tabular-nums">{fmtPKR(p.safe_cash_amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
